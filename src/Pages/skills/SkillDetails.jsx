import { useNavigate } from "react-router";
import Button from "../../Components/Button";
import ResourceCard from "../../Components/ResourceCard";

const SkillDetails = () => {
    const jobs = ["UI/UX Designer", "Product Designer", "Consultant"];
    const companies = ["Google", "Apple", "Microsoft", "Adobe", "Meta"];
    const relatedRes = [
      {
        postedby: "name surname",
        type: "document",
        images: [
          "https://images.pexels.com/photos/4016579/pexels-photo-4016579.jpeg",
          "https://images.pexels.com/photos/2539151/pexels-photo-2539151.jpeg",
        ],
        userImage:
          "data:image/jpeg;base64,",
        link: null,
        specialization: "abc",
        likes: 2,
        postTime: "2023-01-01 19:25:03",
        liked: true,
      },
    ];
    const navigate = useNavigate();
  return (
    <div className="w-full h-full rounded-[20px] smallShadow p-[20px] space-y-[30px]">
      <img
        src="https://sm.pcmag.com/pcmag_uk/review/f/figma/figma_kevp.png"
        className="rounded-[15px] h-[150px] w-full object-cover"
      />
      {/**Skill Title and Description*/}
      <div className="space-y-[15px]">
        <h1 className="font-bold H-18">Master UI/UX Design with Figma</h1>
        <p className="H-14 text-text2">
          Gain industry-recognized certification that validates your ability to
          design user-friendly, professional, and scalable digital products
          using Figma. Learn everything from wireframing and prototyping to
          advanced collaboration tools—making you job-ready for the fast-growing
          design industry.
        </p>
      </div>
      {/**Related jobs*/}
      <div className="space-y-[15px]">
        <h1 className="font-bold H-16">
          Possible Jobs Opportunities For Freshers
          <br /> and Experienced
        </h1>
        <div className="flex flex-wrap gap-[10px]">
          {jobs.map((job, index) => (
            <div
              key={index}
              className="bg-secondary rounded-full text-primary min-w-[125px] px-[15px] py-[5px] text-center"
            >
              {job}
            </div>
          ))}
        </div>
      </div>
      {/**Hiring Companies*/}
      <div className="space-y-[5px]">
        <h1 className="font-bold H-16">Hiring Companies</h1>
        <div className="flex flex-wrap gap-[10px] H-14 text-text2">
          {companies.join(", ")}
        </div>
      </div>

      {/**Related Resources*/}
      <div className="space-y-[15px]">
        <div className="H-18 font-bold ">Resources</div>
        <div className="flex flex-col gap-[15px]">
          {relatedRes.map((resource, index) => (
            <ResourceCard key={index} data={resource} />
          ))}
        </div>
      </div>

      <Button text="Earn A Certificate" onClick={() => navigate("/process")} />
    </div>
  );
}

export default SkillDetails