  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      if (
        ...
        sessionStorage.setItem("interviewId", JSON.stringify(response.data.data.sessionId));
      } else {toast.error("Error while parsing CV");
        setLoading(false);
      }
  ...
  return (
    <div className="space-y-[25px]">
      <div className="flex flex-col items-center">
        ...
        <p className="text-text2">Please upload your Resume to continue.</p>
      </div>
      <div className="flex flex-col items-center smallShadow rounded-[20px] bg-white">
        ...
        <div className="w-full p-[15px]">
          <p className="font-semibold ml-[5px] text-text2 mb-[5px]">Job Description</p>
          <textarea
            style={{ boxShadow: "0px 4px 12px 0px #0000001A" }}
            value={jobDetails}
            onChange={(e) => setJobDetails(e.target.value)}
            placeholder="Enter the job description"
            className="w-full min-h-[300px] rounded-[20px] p-[25px] focus:outline-none focus:ring-primary focus:ring-2 resize-none"
            />
        </div>
      </div>
